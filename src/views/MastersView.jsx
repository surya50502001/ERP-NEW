...
export default function MastersView({ onNavigate }) {
  ...
  // Reset view to overview whenever sidebar navigation tab is re-clicked
  useEffect(() => {
    setSubTab('overview');
  ...
  const handleUpdateEditingItem = async () => {
    if (!editingItem || !editingItem.data) return;
    const { type, data } = editingItem;

    if (type === 'country') {
      const id = data.id || data.countryId || data.code;
      await updateCountry(id, data);
    } else if (type === 'state') {
      const id = data.id || data.stateId || data.code;
      await updateState(id, data);
    } else if (type === 'itemType') {
      const id = data.id || data.itemTypeId || data.name;
      await updateItemType(id, data);
    } else if (type === 'brand') {
      const id = data.id || data.brandId || data.name;
      await updateBrand(id, data);
    } else if (type === 'major') {
      const id = data.id || data.majorId;
      await updateMajorCategory(id, data);
    } else if (type === 'sub') {
      const id = data.id || data.subId;
      await updateSubCategory(id, data);
    } else if (type === 'subSub') {
      const id = data.id || data.subSubId;
      await updateSubSubCategory(id, data);
    } else if (type === 'uom') {
      const id = data.id || data.uomId || data.code;
      await updateUOM(id, data);
    } else if (type === 'party') {
      const id = data.id || data.partyId;
      const formattedAddress = [data.addr1, data.addr2, data.addr3, data.addr4].filter(Boolean).join(', ');
  ...
  // Helper renderer for Universal Edit Modal
  const renderEditModal = () => {
    ...
    return (
      <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 overflow-y-auto">
        <div className="bg-white rounded-2xl max-w-xl w-full shadow-2xl overflow-hidden border border-slate-200 my-8 text-xs animate-in fade-in zoom-in-95">
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
            <div>
              <h3 className="text-sm font-bold text-slate-900">
                Edit {type === 'country' ? 'Country' : type === 'state' ? 'State' : type === 'itemType' ? 'Item Type' : type === 'brand' ? 'Brand' : type === 'major' ? 'Major Category Group' : type === 'sub' ? 'Sub Category Group' : type === 'subSub' ? 'Sub-Sub Category Group' : type === 'uom' ? 'Unit of Measure' : type === 'party' ? 'Party Master' : 'Product Master'}
              </h3>
              <p className="text-[11px] text-slate-500 font-mono mt-0.5">ID: {data.id || data.countryId || data.stateId || data.productId || data.partyId || data.code || 'Record'}</p>
            </div>
            <button onClick={() => setEditingItem(null)} className="p-1.5 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-lg">
          ...
          <div className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
            ...
            {type === 'country' && (
              <div className="space-y-3">
                <Input label="Country Name *" value={data.name || ''} onChange={e => setField('name', e.target.value)} />
                <Input label="ISO Code *" value={data.code || ''} onChange={e => setField('code', e.target.value.toUpperCase())} />
                <Input label="Currency Code" value={data.currencyCode || ''} onChange={e => setField('currencyCode', e.target.value.toUpperCase())} />
                <Input label="Phone Code" value={data.phoneCode || ''} onChange={e => setField('phoneCode', e.target.value)} />
                <Select label="Status" options={['Active', 'Inactive']} value={data.status || 'Active'} onChange={e => setField('status', e.target.value)} />
              </div>
            )}
            ...
            {type === 'state' && (
              <div className="space-y-3">
                <Input label="State Name *" value={data.name || ''} onChange={e => setField('name', e.target.value)} />
                <Input label="State Code *" value={data.code || ''} onChange={e => setField('code', e.target.value.toUpperCase())} />
                <div>
                  <label className="block font-semibold text-slate-700 mb-1">Country *</label>
                  <select
                    value={data.countryCode || 'IND'}
                ...
                <Input label="GST State Code" value={data.gstStateCode || ''} onChange={e => setField('gstStateCode', e.target.value)} />
                <Select label="Status" options={['Active', 'Inactive']} value={data.status || 'Active'} onChange={e => setField('status', e.target.value)} />
              </div>
            )}
            ...
            {type === 'itemType' && (
              <div className="space-y-3">
                <Input label="Item Type Name *" value={data.name || ''} onChange={e => setField('name', e.target.value)} />
                <Input label="Type Code" value={data.code || ''} onChange={e => setField('code', e.target.value.toUpperCase())} />
              </div>
            )}
            ...
            {type === 'brand' && (
              <div className="space-y-3">
                <Input label="Brand Name *" value={data.name || ''} onChange={e => setField('name', e.target.value)} />
              </div>
            )}
            ...
            {type === 'major' && (
              <div className="space-y-3">
                <Input label="Major Group Name *" value={data.name || ''} onChange={e => setField('name', e.target.value)} />
                <Input label="Code" value={data.code || ''} onChange={e => setField('code', e.target.value.toUpperCase())} />
              </div>
            )}
            ...
            {type === 'sub' && (
              <div className="space-y-3">
                <Input label="Sub Group Name *" value={data.name || ''} onChange={e => setField('name', e.target.value)} />
                <div>
                  <label className="block font-semibold text-slate-700 mb-1">Parent Major Category Group</label>
                  <select
                    value={data.majorId || ''}
                    onChange={e => setField('majorId', e.target.value)}
                    className="w-full px-3 py-2 border border-slate-300 rounded-lg text-xs bg-white focus:outline-hidden focus:ring-1 focus:ring-slate-900"
                  >
                    {(state.categories?.major || []).map(m => (
            ...
            {type === 'subSub' && (
              <div className="space-y-3">
                <Input label="Sub-Sub Group Name *" value={data.name || ''} onChange={e => setField('name', e.target.value)} />
                <div>
                  <label className="block font-semibold text-slate-700 mb-1">Parent Sub Category Group</label>
                  <select
                    value={data.subId || ''}
                    onChange={e => setField('subId', e.target.value)}
                    className="w-full px-3 py-2 border border-slate-300 rounded-lg text-xs bg-white focus:outline-hidden focus:ring-1 focus:ring-slate-900"
                  >
                    {(state.categories?.sub || []).map(s => (
            ...
            {type === 'uom' && (
              <div className="space-y-3">
                <Input label="UOM Code *" value={data.code || ''} onChange={e => setField('code', e.target.value.toUpperCase())} />
                <Input label="UOM Full Name *" value={data.name || ''} onChange={e => setField('name', e.target.value)} />
                <Input label="Decimal Places" type="number" value={data.decimalPlaces ?? 2} onChange={e => setField('decimalPlaces', parseInt(e.target.value) || 0)} />
              </div>
            )}
            ...
            {type === 'party' && (
              <div className="space-y-3">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <Input label="Party Name *" value={data.name || ''} onChange={e => setField('name', e.target.value)} />
                  <Select label="Party Type" options={['Supplier', 'Customer', 'Both']} value={data.partyType || data.type || 'Supplier'} onChange={e => setField('partyType', e.target.value)} />
                  <Input label="Phone Number" value={data.phone || data.contactNumber || ''} onChange={e => { setField('phone', e.target.value); setField('contactNumber', e.target.value); }} />
                  <Input label="Email" value={data.email || ''} onChange={e => setField('email', e.target.value)} />
                  <Input label="GSTIN Number" value={data.gstin || ''} onChange={e => setField('gstin', e.target.value)} />
                  <Input label="PAN Number" value={data.pan || ''} onChange={e => setField('pan', e.target.value)} />
                </div>

                <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 space-y-2">
                  <div className="font-bold text-slate-800 text-[11px] uppercase">4-Line Address Breakdown</div>
                  <Input label="Address Line 1" placeholder="Door No / Building" value={data.addr1 || ''} onChange={e => setField('addr1', e.target.value)} />
                  <Input label="Address Line 2" placeholder="Street / Road" value={data.addr2 || ''} onChange={e => setField('addr2', e.target.value)} />
                  <Input label="Address Line 3" placeholder="Landmark / Area" value={data.addr3 || ''} onChange={e => setField('addr3', e.target.value)} />
                  <Input label="Address Line 4" placeholder="Taluk / District" value={data.addr4 || ''} onChange={e => setField('addr4', e.target.value)} />
                  
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-1">
                    <Input label="City" value={data.city || ''} onChange={e => setField('city', e.target.value)} />
                    <div>
                      <label className="block font-semibold text-slate-700 mb-1">Country</label>
                      <select
                    ...
                    <Input label="Pincode" value={data.pincode || ''} onChange={e => setField('pincode', e.target.value)} />
                  </div>
                </div>
            ...
            {type === 'product' && (
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <Input label="Product Name *" value={data.name || ''} onChange={e => setField('name', e.target.value)} />
                  <div>
                    <label className="block font-semibold text-slate-700 mb-1">Item Type</label>
                    <select
                      value={data.itemType || itemTypeOptions[0]}
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <label className="block font-semibold text-slate-700 mb-1">Major Group</label>
                    <select
                      value={data.majorGroup || majorOptions[0]}
                      onChange={e => setField('majorGroup', e.target.value)}
                      className="w-full px-3 py-2 border border-slate-300 rounded-lg text-xs bg-white"
                    >
                  ...
                  <div>
                    <label className="block font-semibold text-slate-700 mb-1">Sub Group</label>
                    <select
                      value={data.subGroup || subOptions[0]}
                  ...
                  <div>
                    ...
                    <select
                      value={data.brand || brandOptions[0]}
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <Input label="Purchase Rate (₹)" type="number" value={data.purchaseRate ?? data.avgRate ?? 0} onChange={e => setField('purchaseRate', parseFloat(e.target.value) || 0)} />
                  <Input label="Selling Price (₹)" type="number" value={data.sellingPrice ?? 0} onChange={e => setField('sellingPrice', parseFloat(e.target.value) || 0)} />
                </div>

                <div className="grid grid-cols-3 gap-3">
                  <Input label="HSN Code" value={data.hsnCode || ''} onChange={e => setField('hsnCode', e.target.value)} />
                  <Select label="GST Rate %" options={[0, 5, 12, 18, 28]} value={data.gstRate ?? 5} onChange={e => setField('gstRate', parseFloat(e.target.value) || 0)} />
                  <Input label="Min Reorder Level" type="number" value={data.minReorderLevel ?? 50} onChange={e => setField('minReorderLevel', parseFloat(e.target.value) || 0)} />
                </div>
          ...
          <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-end gap-3 bg-slate-50/50">
            ...
            <Button variant="primary" size="sm" onClick={handleUpdateEditingItem}>
              ...
              {filteredCountries.map((c, idx) => (
                <tr key={idx} className="hover:bg-slate-50/50">
                  ...
                  <td className="py-3 px-4 text-right whitespace-nowrap">
                    <button
                      onClick={() => setEditingItem({ type: 'country', data: { ...c } })}
                      className="p-1 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 rounded transition-colors mr-1"
                      title="Edit Country"
                    >
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 font-medium text-slate-800">
              {(state.itemTypes || []).map((it, idx) => {
                ...
                const id = typeof it === 'string' ? it : (it.id || it.itemTypeId || it.name);
                return (
                  <tr key={idx} className="hover:bg-slate-50/50">
                    ...
                    <td className="py-3 px-4 text-right whitespace-nowrap">
                      <button
                        onClick={() => setEditingItem({ type: 'itemType', data: { id, name, code } })}
                        className="p-1 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 rounded transition-colors mr-1"
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 font-medium text-slate-800">
              {(state.brands || []).map((b, idx) => {
                ...
                const id = typeof b === 'string' ? b : (b.id || b.brandId || b.name);
                return (
                  <tr key={idx} className="hover:bg-slate-50/50">
                    <td className="py-3 px-4 font-semibold text-slate-900">{name}</td>
                    <td className="py-3 px-4 text-right whitespace-nowrap">
                      <button
                        onClick={() => setEditingItem({ type: 'brand', data: { id, name } })}
                        className="p-1 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 rounded transition-colors mr-1"
                        title="Edit Brand"
                      >
                        <Icon name="Edit" className="w-4 h-4" />
            ...
            <div key={mj.id} className="border border-slate-200 rounded-xl p-4 bg-slate-50/50 space-y-3">
              <div className="flex items-center justify-between border-b border-slate-200 pb-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => setEditingItem({ type: 'major', data: { ...mj } })}
                    className="p-1 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 rounded transition-colors"
                    title="Edit Major Group"
                  >
                    <Icon name="Edit" className="w-3.5 h-3.5" />
                  </button>
                  <button onClick={() => deleteMajorCategory(mj.id)} className="p-1 text-rose-500 hover:bg-rose-100 rounded" title="Delete Major Group">
                  ...
                  <div key={sb.id} className="bg-white p-3 rounded-lg border border-slate-200 space-y-2">
                    <div className="flex items-center justify-between font-bold text-slate-800">
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          onClick={() => setEditingItem({ type: 'sub', data: { ...sb } })}
                          className="p-1 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 rounded transition-colors"
                          title="Edit Sub Group"
                        >
                          <Icon name="Edit" className="w-3.5 h-3.5" />
                        </button>
                        <button onClick={() => deleteSubCategory(sb.id)} className="p-1 text-rose-500 hover:bg-rose-50 rounded" title="Delete Sub Group">
                          <Icon name="Trash2" className="w-3.5 h-3.5" />
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="pl-6 space-y-1 pt-1">
                      {(state.categories?.subSub || []).filter(ssb => ssb.subId === sb.id).map(ssb => (
                        <div key={ssb.id} className="flex items-center justify-between text-slate-600 bg-slate-50 px-2.5 py-1 rounded border border-slate-100">
                          ...
                          <div className="flex items-center gap-1">
                            <button
                              onClick={() => setEditingItem({ type: 'subSub', data: { ...ssb } })}
                              className="text-slate-400 hover:text-indigo-600 p-0.5 rounded"
                              title="Edit Sub-Sub Group"
                            >
                              <Icon name="Edit" className="w-3 h-3" />
                            </button>
                            <button onClick={() => deleteSubSubCategory(ssb.id)} className="text-rose-500 hover:text-rose-700 p-0.5 rounded" title="Delete Sub-Sub Group">
            ...
            <tbody className="divide-y divide-slate-100 font-medium text-slate-800">
              {(state.uoms || []).map(u => (
                <tr key={u.id} className="hover:bg-slate-50/50">
                  ...
                  <td className="py-3 px-4 text-right whitespace-nowrap">
                    <button
                      onClick={() => setEditingItem({ type: 'uom', data: { ...u } })}
                      className="p-1 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 rounded transition-colors mr-1"
                      title="Edit UOM"
                    >
                      <Icon name="Edit" className="w-4 h-4" />
                    </button>
                    <button onClick={() => deleteUOM(u.id || u.uomId || u.code)} className="p-1 text-rose-500 hover:text-rose-700 hover:bg-rose-50 rounded transition-colors" title="Delete UOM">
                      <Icon name="Trash2" className="w-4 h-4" />
                    </button>
                  </td>
                </tr>
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 font-medium text-slate-800">
              {filteredParties.length === 0 ? (
                <tr>
                  <td colSpan="7" className="py-8 text-center text-slate-400 italic">No parties found. Click "Add Party" above to create one.</td>
                </tr>
              ) : (
                filteredParties.map(p => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-slate-50/80 align-top">
                      ...
                      <td className="py-3 px-4 text-right whitespace-nowrap">
                        <button
                          onClick={() => setEditingItem({ type: 'party', data: { ...p } })}
                          className="p-1 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 rounded transition-colors mr-1"
                          title="Edit Party"
                        >
                          <Icon name="Edit" className="w-4 h-4" />
                        </button>
                        <button onClick={() => deleteParty(p.id || p.partyId || p.name)} className="p-1 text-rose-500 hover:text-rose-700 hover:bg-rose-50 rounded transition-colors" title="Delete Party">
                          <Icon name="Trash2" className="w-4 h-4" />
                        </button>
                      </td>
                    </tr>
  ...
  if (subTab === 'products') {
    const filteredProds = (state.products || []).filter(p => p.name.toLowerCase().includes(prodSearch.toLowerCase()) || (p.id && String(p.id).toLowerCase().includes(prodSearch.toLowerCase())));
    ...
    return (
      <div className="space-y-6 max-w-7xl mx-auto">
        ...
        {isProdFormOpen && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 overflow-y-auto">
            <div className="bg-white rounded-2xl max-w-2xl w-full shadow-2xl overflow-hidden border border-slate-200 my-8 animate-in fade-in zoom-in-95">
              <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <div>
        ...
        {quickAddType && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-xl max-w-sm w-full p-5 shadow-2xl border border-slate-200 space-y-4 text-xs animate-in fade-in zoom-in-95">
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 font-medium text-slate-800">
              {filteredProds.length === 0 ? (
                <tr>
                  <td colSpan="8" className="py-8 text-center text-slate-400 italic">No items found. Click "Add Item Master" above to create one.</td>
                </tr>
              ) : (
                filteredProds.map(p => (
                  <tr key={p.id} className="hover:bg-slate-50/80">
                    ...
                    <td className="py-3 px-4 text-right whitespace-nowrap">
                      ...
                      <button onClick={() => deleteProduct(p.id || p.productId || p.name)} className="p-1 text-rose-500 hover:text-rose-700 hover:bg-rose-50 rounded transition-colors" title="Delete Product">
                        <Icon name="Trash2" className="w-4 h-4" />
                      </button>
                    </td>
                  </tr>
